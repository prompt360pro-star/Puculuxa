...
describe('AuthService', () => {
    ...
    describe('validateUser', () => {
        it('should return user without password if credentials are valid', async () => {
            const user = { id: '1', email: 'test@test.com', password: 'hashedpassword', role: 'CUSTOMER' };
    ...
    describe('login', () => {
        it('should return access token, refresh token and user info', async () => {
            const user = { id: '1', email: 'test@test.com', name: 'Test User', role: 'CUSTOMER' };
            ...
            expect(result).toEqual({
                ...
                user: { id: '1', email: 'test@test.com', name: 'Test User', role: 'CUSTOMER' },
            });
            expect(mockJwtService.sign).toHaveBeenCalledWith({ email: 'test@test.com', sub: '1', role: 'CUSTOMER' });
            expect(mockJwtService.sign).toHaveBeenCalledWith({ email: 'test@test.com', sub: '1', role: 'CUSTOMER' }, { expiresIn: '7d' });
    ...
    describe('refreshToken', () => {
        it('should return new tokens if refresh token is valid', async () => {
            const user = { id: '1', email: 'test@test.com', password: 'pwd', refreshToken: 'hashed-token', role: 'CUSTOMER' };
            mockPrismaService.user.findUnique.mockResolvedValue(user);
            (bcrypt.compare as jest.Mock).mockResolvedValue(true);
        ...
        it('should return null if refresh token does not match', async () => {
            mockPrismaService.user.findUnique.mockResolvedValue({ id: '1', refreshToken: 'hashed' });
            (bcrypt.compare as jest.Mock).mockResolvedValue(false);
    ...
    describe('register', () => {
        it('should hash password and create user returning data without password', async () => {
            const registerDto = { email: 'new@test.com', password: 'password', name: 'New User', phone: '123' };
            const createdUser = { id: '2', ...registerDto, password: 'hashedpassword', role: 'CUSTOMER' };
            ...
            expect(result).toEqual({ id: '2', email: 'new@test.com', name: 'New User', phone: '123', role: 'CUSTOMER' });
